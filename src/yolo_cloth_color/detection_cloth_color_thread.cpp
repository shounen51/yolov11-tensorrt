...
namespace YoloWithClothColor {
    std::unique_ptr<YOLOv11> model;
    std::unique_ptr<ClothesSegDetector> clothesModel;
    ...
    void createModelAndStartThread(const char* engine_path, const char* clothes_engine_path, int camera_amount, float conf_threshold, const char* logFilePath) {
        ...
        clothesModel = std::make_unique<ClothesSegDetector>(clothes_engine_path, 0.3f, logger);
            ...
            for (int i = 0; i < count; ++i) {
                ...
                if (det.class_id == static_cast<int>(CustomClass::PERSON)) {
                    ...
                    if (personCrop.empty() || personCrop.cols <= 0 || personCrop.rows <= 0) {
                        AILOG_DEBUG("frame:" + std::to_string(frame_counter) + " skip empty/invalid personCrop");
                        continue;
                    }
                    size_t rowBytes = static_cast<size_t>(personCrop.cols) * personCrop.elemSize(); // cols * channels
                    size_t heightRows = static_cast<size_t>(personCrop.rows);
                    size_t imgBytes = rowBytes * heightRows;
                    if (cudaMalloc(&dImage, imgBytes) != cudaSuccess) {
                        AILOG_ERROR("cudaMalloc failed for input image");
                        continue;
                    }
                    // Use 2D copy to handle non-contiguous Mat (step may differ from rowBytes)
                    if (cudaMemcpy2D(dImage, rowBytes, personCrop.data, personCrop.step, rowBytes, heightRows, cudaMemcpyHostToDevice) != cudaSuccess) {
                        AILOG_ERROR("cudaMemcpy2D failed for input image");
                        cudaFree(dImage);
                        continue;
                    }
                    clothesModel->preprocess(dImage, personCrop.cols, personCrop.rows, true);
                    clothesModel->inference();
                    std::vector<Segmentation> results;
                    clothesModel->postprocess(results);
                    cudaFree(dImage);
                    for (auto& seg : results) {
                        if (upperColorLabel == "none" && clothesModel->isUpperCloth(seg.classId)) { // 假設 classId 0 是衣服
                            cv::Mat resizedMask;
                            cv::resize(seg.mask, resizedMask, personCrop.size());
                            cv::Mat mask_binary;
                            cv::threshold(resizedMask, mask_binary, 127, 255, cv::THRESH_BINARY);
                            vector<unsigned char> color = colorClassifier.classifyStatistics(personCrop, 500, cv::COLOR_RGB2HSV, mask_binary);
                            int maxIndex = 0;
                            int maxCount = 0;
                        ...
                        else if (lowerColorLabel == "none" && clothesModel->isLowerCloth(seg.classId)) { // 假設 classId 1 是褲子
                            cv::Mat resizedMask;
                            cv::resize(seg.mask, resizedMask, personCrop.size());
                            cv::Mat mask_binary;
                            cv::threshold(resizedMask, mask_binary, 127, 255, cv::THRESH_BINARY);
