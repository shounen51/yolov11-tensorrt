...
namespace climb {
    ...
    // 檢查 MRTRedlightROI 的顏色比例，如果紅色+白色超過15%則返回true
    bool checkMRTRedlightROI(int camera_id, uint8_t* gpu_rgb_buffer, int width, int height) {
        if (MRTRedlightROI_map.find(camera_id) == MRTRedlightROI_map.end()) {
            return false; // 沒有找到對應的相機ID
        }

        if (MRTRedlightROI_map[camera_id].find(static_cast<int>(functions::CLIMB)) == MRTRedlightROI_map[camera_id].end()) {
            return false; // 沒有找到對應的功能ID
        }

        for (auto& roi_pair : MRTRedlightROI_map[camera_id][static_cast<int>(functions::CLIMB)]) {
            MRTRedlightROI& redlight_roi = roi_pair.second;

            // 從原圖切出 ROI
            cv::Rect roi_rect(redlight_roi.left_top.x, redlight_roi.left_top.y,
                             redlight_roi.right_bottom.x - redlight_roi.left_top.x,
                             redlight_roi.right_bottom.y - redlight_roi.left_top.y);

            // 確保 ROI 在圖像範圍內
            roi_rect.x = std::max(0, roi_rect.x);
            roi_rect.y = std::max(0, roi_rect.y);
            roi_rect.width = std::min(roi_rect.width, width - roi_rect.x);
            roi_rect.height = std::min(roi_rect.height, height - roi_rect.y);

            if (roi_rect.width > 0 && roi_rect.height > 0) {
                // 從 GPU RGB buffer 取得 ROI 圖像
                std::vector<uint8_t> rgb_host(width * height * 3);
                CUDA_CHECK(cudaMemcpy(rgb_host.data(), gpu_rgb_buffer, width * height * 3 * sizeof(uint8_t), cudaMemcpyDeviceToHost));
                cv::Mat rgb_image(height, width, CV_8UC3, rgb_host.data());
                cv::Mat roi_image = rgb_image(roi_rect);

                // 從 MRTRedlightROI.mask 切出對應的 mask
                cv::Mat roi_mask;
                if (!redlight_roi.mask.empty()) {
                    // 使用和 roi_rect 相同的位置從 mask 中取出對應區域
                    cv::Rect mask_rect(roi_rect.x, roi_rect.y, roi_rect.width, roi_rect.height);

                    if (mask_rect.width > 0 && mask_rect.height > 0) {
                        roi_mask = redlight_roi.mask(mask_rect);
                    }
                }

                // 使用 ColorClassifier 分析顏色
                HSVColorClassifier colorClassifier;
                colorClassifier.setDefaultColorRange();

                std::vector<unsigned char> color_stats = colorClassifier.classifyStatistics(
                    roi_image,
                    1000,  // sample number
                    cv::COLOR_RGB2HSV,  // conversion code
                    roi_mask,  // mask
                    true    // use percent
                );

                // 檢查紅色 (index 4) 和白色 (index 3) 的百分比
                int red_percent = (4 < color_stats.size()) ? color_stats[4] : 0;
                int white_percent = (3 < color_stats.size()) ? color_stats[3] : 0;
                int total_red_white_percent = red_percent + white_percent;

                AILOG_DEBUG("MRT Redlight ROI " + std::to_string(roi_pair.first) +
                          " - Red: " + std::to_string(red_percent) + "%, White: " + std::to_string(white_percent) +
                          "%, Total: " + std::to_string(total_red_white_percent) + "%");

                if (total_red_white_percent > 15) {
                    return true; // 找到一個超過閾值的就返回true
                }
            }
        }
